...
DhistMinSteps = 4;
% input the data
fileall = [3]; % input the index of field of view 
cellpro = [3 4 5 6 7]; % 
...
%function GFPseg()
if fileidx < 100
    BF_filename = sprintf('dwell_64_20230912_YHZ75_0%d_w2T2_merge_GFP-%d.mat',fileidx,celltypeiter);
    BF_file= imread(BF_filename);
    BF_file= BF_file(:,:,1);
    BF_oriname = sprintf('dwell_64_20230912_YHZ75_0%d_w1BF.TIF',fileidx);
    BF_ori = imread(BF_oriname);
    savename = strrep(BF_filename,'GFP','BF');
    GFPfilename = sprintf('dwell_64_20230912_YHZ75_0%d_w2T2 GFP.TIF',fileidx);
    trackfile{1} = sprintf('stream%d_dwell_64_20230912_YHZ75_0%d.tif_spots.csv',fileidx,fileidx);
