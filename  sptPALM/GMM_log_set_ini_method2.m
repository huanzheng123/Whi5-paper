function [DataInx,GMMboundfraction,GMMboundmu] = GMM_log_set_ini_method2 (data,filename,numcomp)
% set localparameter
%GMM_models_tested = 4;
...
% plot the histogram 
BoundNumAll = find (DataInx(:,9) == DMeansortInx(1));
boundplot = DataInx(BoundNumAll,:);

BoundNumAll2 = find (DataInx(:,9) == DMeansortInx(2));
boundplot2 = DataInx(BoundNumAll2,:);

dataplot = log(boundplot(:,8));
dataplot2 = log(boundplot2(:,8));
figure
