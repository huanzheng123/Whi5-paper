...
DhistMinSteps = 4;
% input the data
fileall = [1 2 3 4 5 6 7]; % input the index of field of view 
cellpro = [3 4 5 6 7]; % 
...
%function GFPseg()
if fileidx < 10
    BF_filename = sprintf('dwell_61_202309011_MSJ63_%d_w2T2_merge_GFP-%d.mat',fileidx,celltypeiter);
    BF_file= imread(BF_filename);
    BF_file= BF_file(:,:,1);
    BF_oriname = sprintf('dwell_61_202309011_MSJ63_%d_w1BF.TIF',fileidx);
    BF_ori = imread(BF_oriname);
    savename = strrep(BF_filename,'GFP','BF');
    GFPfilename = sprintf('dwell_61_202309011_MSJ63_%d_w2T2 GFP.tif',fileidx);
    trackfile{1} = sprintf('dwell_61_202309011_MSJ63_%d_w2T2 GFP.csv',fileidx);
